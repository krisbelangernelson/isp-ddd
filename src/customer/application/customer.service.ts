...
import type { AccessToken, CustomerBody, CustomerExists, CustomerType, Login, LoginResponse } from '../domain/customer'

export const registerCustomer = (repo: CustomerRepository) => async (body: CustomerBody): Promise<CustomerType> => {
  const created = await repo.save(body)
  return created
}

export const loginCustomer = (repo: CustomerRepository) => async (body: Login): Promise<LoginResponse> => {
...
export const customerExists = (repo: CustomerRepository) => async (body: CustomerExists): Promise<{ emailExists: boolean; phoneExists: boolean }> => {
...
export const customerArea = (repo: CustomerRepository) => async (token: AccessToken | undefined): Promise<CustomerBody | object> => {
  if (token === undefined) {
    return {}
  }
